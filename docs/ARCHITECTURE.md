...
Embeddings use the 384-dimensional `q4` artifact from `onnx-community/all-MiniLM-L6-v2-ONNX` through the local Transformers runtime. The writable daemon enables extension loading, initializes the sqlite-vec table, and starts one shared background model warm-up at startup; concurrent checkpoint/search requests reuse that load. Read-only MCP loads sqlite-vec and uses the initialized vector table when present. During warm-up state reports that the model is initializing; sqlite-vec or embedding-model failure is non-fatal and explicitly reports `fts_graph` while retrieval continues with FTS5, graph, importance, and recency. The first normal-app launch downloads the weights unless already cached. `bootstrap.sh --demo` intentionally disables embeddings for deterministic replay.
