...
export function prepareTerminalSession(state, policy) {
  if (!policy.sources.terminal || !policy.metadata.personalMetadata) return undefined;
  if (
    Number.isFinite(Number(state.startedAtMs)) &&
    Number(state.startedAtMs) < Date.now() - policy.retentionHours * 60 * 60 * 1_000
  ) return undefined;
  if (
    typeof state.cwd === "string" &&
    policy.ignoredPathPatterns.some((pattern) => globMatchesPath(state.cwd, pattern))
  ) return undefined;
  const prepared = { ...state };
  if (!policy.metadata.relativeFilePaths) delete prepared.cwd;
  if (!policy.metadata.commandNames) delete prepared.shape;
  return prepared;
}

export function applyTerminalPolicy(event, policy) {
  if (!policy.sources.terminal) return undefined;
  ...
  if (
    typeof event.attributes?.cwd === "string" &&
    policy.ignoredPathPatterns.some((pattern) => globMatchesPath(event.attributes.cwd, pattern))
  ) return undefined;
  const attributes = { ...(event.attributes ?? {}) };
  if (!policy.metadata.personalMetadata) delete attributes.projectName;
  if (!policy.metadata.relativeFilePaths) delete attributes.cwd;
  if (!policy.metadata.commandNames) {
    delete attributes.commandShape;
    delete attributes.executable;
    delete attributes.subcommand;
  }
  if (!policy.metadata.commandFlagNames) delete attributes.flags;
  return {
    ...event,
    title: !policy.metadata.commandNames && event.eventType !== "privacy.drop.aggregate"
