...
test("physically evicts expired and invalid Git queue entries", async (context) => {
  const queueDir = await mkdtemp(path.join(os.tmpdir(), "continuum-git-queue-"));
...
test("retains only the newest events at the documented Git cap", async (context) => {
  const queueDir = await mkdtemp(path.join(os.tmpdir(), "continuum-git-cap-"));
  context.after(() => rm(queueDir, { recursive: true, force: true }));
  const nowMs = Date.parse("2026-07-18T12:00:00.000Z");
...
test("Git queue honors a tightened one-hour policy", async (context) => {
  const queueDir = await mkdtemp(path.join(os.tmpdir(), "continuum-git-policy-retention-"));
  context.after(() => rm(queueDir, { recursive: true, force: true }));
  const nowMs = Date.parse("2026-07-18T12:00:00.000Z");
